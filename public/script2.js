document.addEventListener("DOMContentLoaded", function () {
  ...
  function downloadPDF() {
    try {
      ...
      // Title
      doc.setFontSize(18);
      doc.text("Amortization Details (Month-wise)", pageWidth / 2, 15, {
        align: "center",
      });
  ...
  function calculateEMI() {
    ...
    document.getElementById("interestValue").innerText = formatCurrency(
      Math.round(totalInterest),
    );
    document.getElementById("emiValue").innerText = formatCurrency(
      Math.round(emi),
    );
    document.getElementById("totalPaymentValue").innerText = formatCurrency(
    ...
    function updateSliderFill(slider) {
      ...
      slider.style.background = `linear-gradient(to right, #22d3ee ${percent}%, #334155 ${percent}%)`;
    }
